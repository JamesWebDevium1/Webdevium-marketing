...
export default function ClutchReviewBadge({ rating = 5.0 }: ClutchReviewBadgeProps) {
  const star = (
    <svg viewBox="0 0 20 20" width="28" height="28" aria-hidden="true" className="text-[#f15a29]" fill="currentColor">
  ...
  return (
    <div className="flex items-center gap-3 select-none justify-center h-[110px]">
      <div className="flex flex-col items-center">
        <div className="text-xl uppercase tracking-widest text-gray-600">Reviewed on</div>
        <div className="text-3xl font-extrabold" style={{ color: '#0a192f' }}>Clutch</div>
      </div>
      <div>
          <div className="flex items-center gap-1 text-4xl">{star}{star}{star}{star}{star}</div>
          <div className="text-2xl font-semibold text-gray-700">{rating.toFixed(1)} rating</div>
      </div>
    </div>
  );
}
